...
export default function PhotoGallery({ cemeterySlug, cemeteryName }: PhotoGalleryProps) {
  ...
  useEffect(() => {
    async function fetchPhotos() {
      try {
        const response = await fetch(`/api/photos?slug=${cemeterySlug}`);
          ...
          <div className="grid grid-cols-3 gap-2">
            {photos.slice(0, 6).map((photo, index) => (
              ...
              >
                <Image
                  src={photo.file_url}
                  alt={photo.caption || `Foto van ${cemeteryName}`}
                  fill
        ...
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center">
          ...
          <div className="max-w-5xl max-h-[85vh] w-full h-full relative flex items-center justify-center p-4">
            <Image
              ...
              alt={photos[selectedIndex].caption || `Foto van ${cemeteryName}`}
              fill
