...
// GET: Fetch approved photos for a cemetery
export async function GET(request: NextRequest) {
  ...
  try {
    const photos = await sql`
      SELECT id, cemetery_slug, uploader_name, file_url, caption, created_at, ip_hash
      FROM cemetery_photos
      WHERE cemetery_slug = ${slug} AND status = 'approved'
    ...
    const photosWithPermissions = photos.map(photo => ({
      id: photo.id,
      cemetery_slug: photo.cemetery_slug,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const recentUploads = await sql`
      SELECT COUNT(*) as count FROM cemetery_photos
      WHERE ip_hash = ${ipHash}
      AND cemetery_slug = ${slug}
    ...
    const result = await sql`
      INSERT INTO cemetery_photos (
        cemetery_slug, uploader_name, uploader_email,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await sql`DELETE FROM cemetery_photos WHERE id = ${parseInt(photoId)}`;
