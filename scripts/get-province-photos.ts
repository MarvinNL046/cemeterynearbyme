...
// Get photos for each province from our scraped data
async function getProvincePhotos() {
  ...
  // Collect all photos per province
  ...
  Object.entries(provincePhotos).forEach(([province, photos]) => {
    ...
    console.log(`${province}: ${topPhotos[province].length} photos found`);
  });
  
  // Save the result
  const outputPath = path.join(__dirname, '..', 'data', 'province-photos.json');
