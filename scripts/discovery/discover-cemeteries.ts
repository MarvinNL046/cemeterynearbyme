#!/usr/bin/env npx tsx
/**
 * US Cemetery Discovery Script with Bright Data SERP API
 *
...
const RESULTS_FILE = path.join(DATA_DIR, 'discovered-cemeteries.json');
...
/**
 * Process SERP JSON response and extract cemetery data for US
 */
async function processSerpResponse(
  response: { data: any; searchQuery: string },
  location: DiscoveryLocation,
  query: string
): Promise<DiscoveredCemetery[]> {
  const cemeteries: DiscoveredCemetery[] = [];
  ...
  for (const place of places) {
    ...
    const isCemetery =
      ...
      nameLower.includes('graveyard') ||
      nameLower.includes('memorial park') ||
      nameLower.includes('burial') ||
      nameLower.includes('mausoleum') ||
      nameLower.includes('crematorium') ||
      categoryIds.includes('cemetery') ||
      categoryIds.includes('funeral') ||
      categoryIds.includes('memorial') ||
      categoryTitles.includes('cemetery') ||
      categoryTitles.includes('funeral home');

    // For cemetery searches, be more lenient
    const isRelevantQuery = query.toLowerCase().includes('cemetery') ||
                           query.toLowerCase().includes('burial') ||
                           query.toLowerCase().includes('memorial');
    if (!isCemetery && !isRelevantQuery) continue;

    // Get primary category
    const primaryCategory = categories[0]?.title || categories[0]?.id || 'cemetery';
    ...
    const cemetery: DiscoveredCemetery = {
 ...
 */
function parseReviews(place: any): DiscoveredCemetery['reviews'] {
  const reviews: DiscoveredCemetery['reviews'] = [];
...
function loadDiscoveredCemeteries(): DiscoveredCemetery[] {
...
function saveDiscoveredCemeteries(cemeteries: DiscoveredCemetery[]): void {
  fs.writeFileSync(RESULTS_FILE, JSON.stringify(cemeteries, null, 2));
}

function updateProgress(locations: DiscoveryLocation[], cemeteries: DiscoveredCemetery[]): void {
  // Group by state
  const stateStats: Record<string, { total: number; completed: number; cemeteries: number }> = {};

  for (const loc of locations) {
    if (!stateStats[loc.state]) {
      stateStats[loc.state] = { total: 0, completed: 0, cemeteries: 0 };
    }
  ...
  for (const cem of cemeteries) {
    if (cem.state && stateStats[cem.state]) {
...
async function main() {
  ...
  console.log('🇺🇸 Cemetery Discovery Script - USA\n');
  ...
  console.log(`   Already found: ${discoveredCemeteries.length} cemeteries`);
  ...
  for (const location of toProcess) {
    ...
    try {
      let locationResults: DiscoveredCemetery[] = [];
